...
import warning from '../warning.mp3'  // Temporarily commented out
...
function Popups() {
  ...
  // const [play, { stop, pause }] = useSound(warning)  // Temporarily disabled
  const play = () => console.log('Audio play triggered')
  const pause = () => console.log('Audio pause triggered')
  const [accident, setAccident] = useState({})

  useEffect(() => {
    console.log('Connecting to Socket.IO at:', process.env.NEXT_PUBLIC_SOCKET_URL || 'http://localhost:4001')
    ...
    socket.on('receive-message', (message) => {
      console.log('🚨 Received accident message:', message)
      if (message) {
        setAccident(message)
        setShow(true)
        play()
