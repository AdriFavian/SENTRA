            ...
            for r in results:
                for c in r.boxes.cls:
                    ...
                    if ("moderate-accident" in class_name or "fatal-accident" in class_name) and not snapshot_taken:
                        ...
                        if "fatal-accident" in class_name:
                            severity = "Fatal"
                        elif "moderate-accident" in class_name:
                            severity = "Serious"
                        else:
                            severity = "Normal"  # Default fallback
                        ...
                        accident_data={
                            ...
                            "ipAddress":ipaddress,
                            "severity": severity,
                            "description": f"Accident detected: {class_name}",
                            "confidence": float(r.boxes.conf[0]) if len(r.boxes.conf) > 0 else 0.8
                            }
                       
                        print(f"Accident data with severity: {accident_data}")
